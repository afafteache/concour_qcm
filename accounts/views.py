    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if profil.role == 'candidat':
                ...
                return _redirect_role(profil.user)
            else:
...
def login_candidat_view(request):
    """Connexion candidat : nom complet + CIN, sans mot de passe."""
    if request.user.is_authenticated:
        return _redirect_role(request.user)
    if request.method == 'POST':
        ...
        if not nom_complet or not cin:
            ...
            try:
                ...
                if nom_db == nom_complet.lower():
                    # Connexion sans mot de passe → backend explicite obligatoire
                    login(request, user,
                          backend='django.contrib.auth.backends.ModelBackend')
...
def register_view(request):
    ...
    - Candidat : pas de mot de passe (connexion par nom + CIN)
    """
    if request.method == 'POST':
        ...
        # Validations de base
        if not all([first_name, last_name, cin]):
            ...
            messages.error(request, 'Un mot de passe est obligatoire pour ce rôle.')
        elif role in ('admin', 'enseignant') and password != password2:
            messages.error(request, 'Les mots de passe ne correspondent pas.')
        else:
            if role == 'candidat':
                # Candidat : mot de passe inutilisable (connexion par nom+CIN)
                user = User.objects.create_user(
                    username=cin,
                    email=email,
                    password=User.objects.make_random_password(),  # inutilisé
            ...
            messages.success(request, f'Compte de {first_name} {last_name} ({role}) créé avec succès.')

            # Redirection selon le rôle de celui qui crée
    ...
    roles_disponibles = ['admin', 'enseignant', 'candidat'] \
        if request.user.profil.role == 'admin' else ['candidat']
